...
const DESKTOP_FOLDER = path.join(REPO_ROOT, 'desktop');
...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	return () => {
		...
		const sources = gulp.src(out + '/**', { base: '.' })
		...
		const packageJsonUpdates = { name, version, pnpm: undefined };
		...
		const productionDependencies = getProductionDependencies(DESKTOP_FOLDER);
		const dependenciesSrc = productionDependencies.map(d => path.relative(REPO_ROOT, d)).map(d => [`${d}/**`, `!${d}/**/{test,tests}/**`]).flat().concat('!**/*.mk');

		const deps = gulp.src(dependenciesSrc, { base: 'desktop', dot: true })
			...
			.pipe(createAsar(path.join(DESKTOP_FOLDER, 'node_modules'), [
				'**/*.node',
				'**/@vscode/ripgrep/bin/*',
