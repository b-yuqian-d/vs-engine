...
function packageTask(type, platform, arch, sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const sources = gulp.src(sourceFolderName + '/**', { base: '.' })
		...
		const packageJsonStream = gulp.src(['remote/package.json'], { base: 'remote' })
			.pipe(json({ version, pnpm: undefined }))
...
['server', 'server-web'].forEach(type => {
		...
		optimize.bundleTask(
			{
				...
				esm: {
					src: 'out-build',
					entryPoints: [
						...(type === 'server' ? serverEntryPoints : serverWithWebEntryPoints),
						...bootstrapEntryPoints
					],
					resources: type === 'server' ? serverResources : serverWithWebResources,
					fileContentMapper: createVSCodeWebFileContentMapper('.build/extensions', type === 'server-web' ? tweakProductForServerWeb(product) : product)
				}
			}
		)
	));
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		['', 'min'].forEach(minified => {
			...
			if (process.platform === platform && process.arch === arch) {
				gulp.task(`vscode-${type}${dashed(minified)}-ci`, serverTaskCI);
			}
			...
			if (process.platform === platform && process.arch === arch) {
				gulp.task(`vscode-${type}${dashed(minified)}`, serverTask);
			}
		});
	});
});
